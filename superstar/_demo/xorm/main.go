...
func newEngine() *xorm.Engine {
	conStr := fmt.Sprintf(ConnStr, UserName, Password, Server, Port, DataBase, url.QueryEscape(Loc))

	engine, err := xorm.NewEngine(DriverName, conStr)
	if err != nil {
		log.Fatal("xorm create engine error", err)
	}
...
func execute() {
	...
	//fmt.Printf("affected : %T , %v", affected, affected)
...
// 字段为空也可以更新（0, 空字符串等）
func ormMustColsUpdate2() {
	info := &UserInfo{Id: 2}
	ok, _ := engine.ID(info.Id).
		MustCols("sys_created").
		Update(&UserInfo{SysUpdated: int(time.Now().Unix())})
	fmt.Printf("ormMustColsUpdate2, rows=%d, sys_created=%d\n", ok, 0)
}
